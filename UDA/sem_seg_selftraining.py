    ...
    with EventStorage(start_iter) as storage:
        if cfg.SOLVER.ALTERNATE_SOURCE_PSEUDOLABELS and dataset_pseudolabels is not None:
            ...
            if cfg.SOLVER.ACTIVATE_CLASSMIX:
                ...
                results_list = training_loop_classmixdatasets(cfg, model, start_iter, max_iter, data_loader,
                ...
                data_loader = build_detection_train_loader(cfg, dataset=dataset, mapper=mapper,
                                                        total_batch_size=cfg.SOLVER.SOURCE_PSEUDOLABELS_BATCH_RATIO[0])
                results_list = training_loop_mixdatasets(cfg, model, start_iter, max_iter, data_loader,
                                                         data_loader_pseudo, storage, optimizer, scheduler,
                                                         periodic_checkpointer, writers, test_dataset_name, epoch,
                                                         model_id)
        else:
            ...
            results_list = training_loop(cfg, model, start_iter, max_iter, data_loader, storage, optimizer, scheduler,
                                         periodic_checkpointer, writers, test_dataset_name, epoch, model_id)
    return results_list


def training_loop(cfg, model, start_iter, max_iter, data_loader, storage, optimizer, scheduler, periodic_checkpointer,
...
def training_loop_mixdatasets(cfg, model, start_iter, max_iter, data_loader, data_loader_pseudo, storage, optimizer,
...
def training_loop_classmixdatasets(cfg, model, start_iter, max_iter, data_loader, data_loader_pseudo, storage,
...
def apply_mpt(outputs, num_classes, tgt_num, tgt_portion, mask_file=None, prior=None, prior_thres=0):
...
def apply_mtp_gt_score(outputs, unlabeled_dataset, num_classes, tgt_num, tgt_portion, mask_file=None, prior=None,
                       prior_thres=0):
    pred_cls_num = np.zeros((tgt_num, outputs[0][0].shape[0], outputs[0][0].shape[1]), dtype=np.uint8)
    pred_conf = np.zeros((tgt_num, outputs[0][0].shape[0], outputs[0][0].shape[1]), dtype=np.float32)
    for index, output in enumerate(outputs):
    ...
    for index in range(tgt_num):
        ...
        conf_matrix = conf_matrix.astype('float') / conf_matrix[:,:19].sum(axis=1)[:, np.newaxis]
        conf_matrix = np.nan_to_num(conf_matrix)
        conf_matrix = np.around(conf_matrix, decimals=2)
        tp = conf_matrix[:,:19].diagonal()
        score = 0
        cls_num = 0
        for i in range(num_classes-1):
            if np.sum(conf_matrix[i,:19]):
                cls_num += 1
                score += tp[i]
        score = score/cls_num
...
def apply_mtp_and_gt_substraction(outputs, unlabeled_dataset, num_classes, tgt_num, tgt_portion, mask_file=None,
                                  prior=None, prior_thres=0, gt_dataset = ''):
    pred_cls_num = np.zeros((tgt_num, outputs[0][0].shape[0], outputs[0][0].shape[1]), dtype=np.uint8)
    pred_conf = np.zeros((tgt_num, outputs[0][0].shape[0], outputs[0][0].shape[1]), dtype=np.float32)
    for index, output in enumerate(outputs):
        pred_cls_num[index] = output[0]
    ...
    for i in range(num_classes-1):
        logger.info("%s: FP mean %.2f%%, std %.2f%% | TP mean %.2f%%, std %.2f%%" % (dict_classes[i], mean_subs[2*i],
                                                                                  ...
                                                                                  std_subs[2*i+1]))
    return pseudolabels, scores_list, pseudolabels_not_filtered, thres, gt_substraction_stats
    ...
    for epoch in range(args.continue_epoch,args.epochs):
        ...
        if pseudolabeling == 'mpt':
            ...
            pseudolabels_A, scores_listA, pseudolabels_A_not_filtered, cls_thresh = \
                apply_mpt(inference_A, cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES, len(unlabeled_datasetA),
                          tgt_portion, args.mask_file, source_priors, prior_thres)
            total_time = time.perf_counter() - start_time
            ...
            pseudolabels_A, scores_listA, pseudolabels_A_not_filtered, cls_thresh, gt_substraction_stats = \
                apply_mtp_and_gt_substraction(inference_A, unlabeled_datasetA, cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,
                                              len(unlabeled_datasetA), tgt_portion, args.mask_file, gt_dataset=gt_dataset)
            total_time = time.perf_counter() - start_time
            logger.info("MPT on unlabeled dataset A: {:.2f} s".format(total_time))
        elif pseudolabeling == 'mtp_gt_score':
            start_time = time.perf_counter()
            pseudolabels_A, scores_listA, pseudolabels_A_not_filtered, cls_thresh = \
                apply_mtp_gt_score(inference_A, unlabeled_datasetA, cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,
                                   len(unlabeled_datasetA), tgt_portion, args.mask_file)
            total_time = time.perf_counter() - start_time
            logger.info("MPT on unlabeled dataset A: {:.2f} s".format(total_time))
        else:
        ...
        coloured_pseudolabels_path_model_A = os.path.join(cfg.OUTPUT_DIR,'model_A',str(epoch),
                                                          'pseudolabeling/coloured_pseudolabels')
        create_folder(coloured_pseudolabels_path_model_A)
        coloured_pseudolabels_not_filtered_path_model_A = os.path.join(cfg.OUTPUT_DIR,'model_A',str(epoch),
        ...
        if len(sorted_unlabeled_datasetA) > cfg.PSEUDOLABELING.NUMBER:
            images_txt_A, psedolabels_txt_A, filenames_and_scoresA = \
                save_pseudolabels(sorted_unlabeled_datasetA[:num_selected], sorted_pseudolabels_A[:num_selected],
                                  sorted_scores_listA[:num_selected], pseudolabels_path_model_A,
                                  coloured_pseudolabels_path_model_A, sorted_pseudolabels_A_not_filtered[:num_selected],
                                  coloured_pseudolabels_not_filtered_path_model_A)
        else:
            if pseudolabeling == 'gt_substraction':
                images_txt_A, psedolabels_txt_A, filenames_and_scoresA = \
                    save_pseudolabels(sorted_unlabeled_datasetA, sorted_pseudolabels_A, sorted_scores_listA,
                                      pseudolabels_path_model_A, coloured_pseudolabels_path_model_A)
            else:
                images_txt_A, psedolabels_txt_A, filenames_and_scoresA = \
                    save_pseudolabels(sorted_unlabeled_datasetA, sorted_pseudolabels_A, sorted_scores_listA,
                                      pseudolabels_path_model_A, coloured_pseudolabels_path_model_A,
        ...
        if not args.only_pseudolabeling:
            ...
            if accumulation_mode is not None and len(accumulated_selection_imgA) > 0:
                if accumulation_mode.lower() == 'all':
                    ...
                    accumulated_selection_pseudoA = merge_txts_and_save(os.path.join(dataset_A_path,
                ...
                if accumulation_mode.lower() == 'update_best_score':
                    accumulated_selection_imgA, accumulated_selection_pseudoA, accumulated_scores_A = \
            ...
            if cfg.SOLVER.ALTERNATE_SOURCE_PSEUDOLABELS:
                ...
                results_A = do_train(cfg, model, weights_train_branchA, dataset_A_source, cfg.DATASETS.TEST_NAME,'a',
                ...
                # Train model A
                logger.info("Training Model A")
                results_A = do_train(cfg, model, weights_train_branchA, dataset_A_name, cfg.DATASETS.TEST_NAME,'a',
            ...
            if epoch < 4 and args.recompute_all_pseudolabels and cfg.SOLVER.ALTERNATE_SOURCE_PSEUDOLABELS:
                ...
                if cfg.PSEUDOLABELING.MODE == 'mpt':
                    start_time = time.perf_counter()
                    pseudolabels_A, scores_listA, pseudolabels_A_not_filtered, cls_thresh = \
                        apply_mpt(inference_A, cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,len(accumulated_pseudoA), tgt_portion,
                                  args.mask_file, source_priors, prior_thres)
                    total_time = time.perf_counter() - start_time
                    logger.info("MPT on unlabeled dataset A: {:.2f} s".format(total_time))
                else:
                    raise Exception('unknown pseudolabeling method defined')
                ...
                # select candidates and save them to add them to the source data
                images_txt_A, psedolabels_txt_A, filenames_and_scoresA = save_pseudolabels(accumulated_imgA,
